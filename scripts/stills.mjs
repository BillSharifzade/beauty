/**
 * Renders the no-WebGL poster and the OpenGraph image from the live 3D scene.
 *
 * Needs a running dev server (npm run dev) and Google Chrome. The page is
 * opened with `?still=lineup`, which the hero section reads to render one
 * authored frame with the page chrome hidden; the section's pinned frame is
 * then screenshotted. Chrome runs on SwiftShader here, so a frame takes
 * seconds rather than milliseconds — the waits are generous.
 *
